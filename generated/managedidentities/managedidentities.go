...
// Command returns the gcloud managedidentities command tree.
				...
				Commands: []*cli.Command{
					...
					},
					{
						...
						Flags: []cli.Flag{
							...
							&cli.StringSliceFlag{Name: "target-dns-ip-addresses", Usage: "The target dns ip addresses.", Required: true},
						},
						Action: func(ctx context.Context, cmd *cli.Command) error {
							name := fmt.Sprintf("projects/%s/locations/%s/domains/%s", cmd.String("project"), cmd.String("location"), cmd.String("domain"))
							client, err := managedidentities.NewClient(ctx)
							...
							req.TargetDnsIpAddresses = cmd.StringSlice("target-dns-ip-addresses")
