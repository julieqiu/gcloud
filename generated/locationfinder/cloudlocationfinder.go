...
// Command returns the gcloud cloudlocationfinder command tree.
				...
				Commands: []*cli.Command{
					{
						...
						Action: func(ctx context.Context, cmd *cli.Command) error {
							...
							client, err := locationfinder.NewCloudLocationFinderClient(ctx)
							if err != nil {
								return err
							}
							defer client.Close()
							pageSize := cmd.Int("page-size")
							req := &locationfinderpb.ListCloudLocationsRequest{Parent: parent}
							if pageSize > 0 {
					...
					},
					{
						...
						Flags: []cli.Flag{
							&cli.StringFlag{Name: "location", Usage: "The location.", Required: true},
							&cli.StringFlag{Name: "cloud_location", Usage: "The cloud_location.", Required: true},
						},
						Action: func(ctx context.Context, cmd *cli.Command) error {
							name := fmt.Sprintf("projects/%s/locations/%s/cloudLocations/%s", cmd.String("project"), cmd.String("location"), cmd.String("cloud_location"))
							client, err := locationfinder.NewCloudLocationFinderClient(ctx)
							if err != nil {
								return err
							}
							defer client.Close()
							req := &locationfinderpb.GetCloudLocationRequest{Name: name}
